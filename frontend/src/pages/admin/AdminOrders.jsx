...
import { 
  Search, Loader2, ShoppingBag, Eye, ChevronDown, MapPin
} from 'lucide-react';
...
export const AdminOrders = () => {
  ...
  const openOrderDetail = async (order) => {
    try {
      const response = await axios.get(`${API}/admin/orders/${order.order_id}`, { withCredentials: true });
              ...
              <div className="flex flex-col lg:flex-row lg:items-center gap-4">
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-3 mb-2">
                    ...
                    <span className="text-sm text-muted-foreground font-mono">{order.order_id}</span>
                  </div>
                ...
                <div className="flex gap-2 overflow-x-auto py-2 lg:py-0">
                  ...
                  {order.items?.length > 3 && (
                    <div className="w-12 h-12 bg-muted rounded-sm flex items-center justify-center text-sm font-medium">
                      +{order.items.length - 3}
                    </div>
                  )}
                </div>
                ...
                <div className="flex items-center justify-between lg:flex-col lg:items-end gap-2">
                  <div className="text-right">
                    ...
                    {order.shipping_cost > 0 && (
                      <p className="text-xs text-muted-foreground">Envío: ${order.shipping_cost?.toFixed(2)}</p>
                    )}
                  </div>
                  <div className="flex gap-2">
                    <Select 
                      value={order.status} 
                      onValueChange={(val) => updateOrderStatus(order.order_id, val)}
                      disabled={updating}
                    >
                      <SelectTrigger className="w-36 rounded-sm text-sm">
                        <SelectValue />
                      </SelectTrigger>
                      <SelectContent>
                        {statusOptions.map(status => (
          ...
          {selectedOrder && (
            <div className="space-y-4">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-mono text-sm text-muted-foreground">{selectedOrder.order_id}</p>
              ...
              <div>
                ...
                <div className="space-y-3">
                  {selectedOrder.items?.map((item, idx) => (
                    <div key={idx} className="flex items-center gap-3 p-2 bg-muted/50 rounded-sm">
                      <img 
                        src={item.image || 'https://via.placeholder.com/48'} 
                        alt={item.name}
              ...
              <div className="space-y-2">
                ...
                <div className="flex justify-between">
                  ...
                  <span>{selectedOrder.shipping_cost > 0 ? `$${selectedOrder.shipping_cost?.toFixed(2)}` : 'Gratis'}</span>
                </div>
